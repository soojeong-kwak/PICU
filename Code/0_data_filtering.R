...
##### 1) exclude 0 row
picu_files_set = 
  list.files('C:/Users/user/Desktop/PICU/PCO2/0 Data/samples_exclude_0_row', full.names = T) %>% 
  str_subset('csv$')
...
for(i in seq_along(picu_files_set)){
  ...
  sub <- fn %>% filter(hn == unique(df$hn), filename == unique(df$filename))
  sub$rows <- n
...
clinic <- unique(na.omit(htwt)) %>% 
  group_by(rid) %>% 
  summarise(wt = round(mean(bwt),2), 
            ht = round(mean(ht),2)) %>%
  dplyr::filter(wt > 0 & ht > 0)

# Age(month) < 18
demo <- admission %>% 
  mutate(adm_age = adm_age_day/365, adm_age_month = adm_age_day/365*12) %>%
  dplyr::filter(adm_age < 18)
demo <- na.omit(demo)

# PaO2 >= 40
abga <- lab %>% dplyr::filter(po2 >= 40)
...
age_list <- demo %>% group_by(rid) %>% summarise(adm_age_day = min(adm_age_day)) %>% left_join(demo[,c(1,4)])
db2 <- tibble()

for (i in 1:nrow(age_list)){
  
  db_sub <- db %>% dplyr::filter(rid == age_list$rid[i])
  demo_sub <- demo %>% dplyr::filter(rid == age_list$rid[i])
  
  # age days+n
  start_dt <- ymd(as_date(min(db_sub$lab_dt)))
  diff_dt <- ymd(as_date(db_sub$lab_dt)) - start_dt
  db_sub$adm_age_day <- min(demo_sub$adm_age_day) + as.numeric(diff_dt)
...
fn_anl <- fn_rows %>% dplyr::filter(hn %in% anl_db$rid)
View(fn_anl)
